<?php
/**
 * E2E render tests for the PHP host pages that mount a jQuery EasyUI treegrid.
 *
 * A Laravel rewrite will replace these pages wholesale; these tests lock down
 ...
 * same way golden fixtures are approved via bin/regen-easyui-fixtures.
 *
...
final class EasyUiHostPageTest extends E2ETestCase
{
    /**
     * Each host page and a string the authenticated rendered body must contain.
     * The marker is the EasyUI treegrid mount point (or, for the org-hierarchy
    ...
    public function test_host_page_renders_treegrid_for_authenticated(string $path, string $marker): void
    {
        ...
        self::assertStringContainsString(
            $marker,
            $body,
            "treegrid mount marker '{$marker}' missing from {$path} — page did not render its EasyUI surface"
        );
    }
    ...
    /**
     * The org-hierarchy page mounts its EasyUI treegrid (<table id='business_units'
     * class='easyui-treegrid'> pointing at /api/v2/organizational_hierarchy/
    ...
    public function test_organizational_hierarchy_treegrid_mount_when_extra_enabled(): void
    {
        ...
        self::assertStringContainsString("id='business_units'", $body, 'business_units treegrid table missing');
        self::assertStringContainsString('easyui-treegrid', $body, 'easyui-treegrid class missing');
        self::assertStringContainsString(
            '/api/v2/organizational_hierarchy/business_unit/tree',
            $body,
            'treegrid data-url missing'
        );
    }
}
