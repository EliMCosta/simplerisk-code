<?php
/**
 * Golden-fixture generator for the EasyUI endpoint contract.
 *
 * NOT part of the normal test run (opt-in via @group regen; excluded by default
 * in tests/bin/test). Hit each EasyUI data endpoint as the test admin, capture
 * its live {status, contentType, envelope}, and (re)write the committed golden
 * under tests/fixtures/easyui/. Run after an INTENTIONAL contract change so the
 * new shape becomes the baseline EasyUiEndpointContractTest checks against:
 *
 *     bin/regen-easyui-fixtures        # host wrapper
 *
 ...
 * @group regen
 */
final class RegenEasyUiFixturesTest extends E2ETestCase
{
    ...
    {
        // Same catalog as EasyUiEndpointContractTest — kept in sync deliberately.
        return EasyUiEndpointContractTest::endpoints();
    }
}
